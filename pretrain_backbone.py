...
class TSN(nn.Module):
    def __init__(self):
        ...
        #self.n_feats = 1000
        #self.n_classes = 200
        #self.l_norm = torch.nn.LayerNorm(self.n_feats) 
        #self.classifier = nn.Parameter(torch.zeros(self.n_classes, self.n_feats))
    ...
    def forward(self, x):
        ...
        ## use cos sim 
        #x = self.l_norm(x)
        #classes = self.l_norm(self.classifier)
        #x = self.l_norm(x)
        #cos_dist = torch.matmul(x, classes.transpose(-2,-1)) 

        
        # average after linear layer
        x = x.reshape(b,s,-1)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    #model = resnet18(pretrained=True, pretrained_model_path="../models/pretrained_resnet.pt.tar")
...
#        ]))
    ...
    #train_dataset = ucf_reader.VideoDataset("/mnt/storage/home/tp8961/scratch/video_datasets/data/somethingsomethingv2_128x96q5_class.zip", "/mnt/storage/home/tp8961/scratch/video_datasets/splits/somethingsomethingv2TrainTestlist", 0, train=True, transform=ucf_reader.img_transform, seq_len=1, single_img=True)
    
    train_dataset = video_reader.VideoDataset(args, meta_batches=False)
    if args.distributed:
...
#        best_acc1 = max(acc1, best_acc1)
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            torch.save(model.module.backbone.state_dict(), os.path.join(args.out_name, "epoch{}.pth.tar".format(epoch+1)))
            ...
            #}, is_best, filename=os.path.join(args.scratch, "pretrained_models/{}_epoch{}.pth.tar".format(args.out_name,epoch+1)))
